...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        ...
        vector<VertexFormat> vertices2
        {
            ...
            VertexFormat(glm::vec3(1, 3, 1), glm::vec3(0, 0, 0))
            // TODO(student): Complete the vertices data for the cube mesh
        ...
        {
            2, 1, 0,    // indices for first triangle
            2, 0, 3
            // TODO(student): Complete indices data for the cube mesh
        };
vector<VertexFormat> vertices4
        ...
        meshes["cube_B"]->InitFromData(vertices2, indices2);

        meshes["sq"] = new Mesh("generated sq");
        meshes["sq"]->InitFromData(vertices3, indices3);

        meshes["sq2"] = new Mesh("generated sq");
        meshes["sq2"]->InitFromData(vertices3, indices4);
        meshes["sq3"] = new Mesh("generated sq");
        meshes["sq3"]->InitFromData(vertices4, indices5);
    }
...
void Lab2::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(indices[0]) * indices.size(), &indices[0], GL_STATIC_DRAW);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1, 0.5f, 1), glm::vec3(0.25f));
    ...
    RenderMesh(meshes["sq"], shaders["VertexColor"], glm::vec3(3, 0.5f, 3), glm::vec3(0.25f));
    RenderMesh(meshes["sq2"], shaders["VertexColor"], glm::vec3(3, 2, 3), glm::vec3(0.25f));
    RenderMesh(meshes["sq3"], shaders["VertexColor"], glm::vec3(3, 2, 3), glm::vec3(0.25f));
